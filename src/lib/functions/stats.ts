...
export const getColumnAverages = (columnDistribution, gamesPlayed: number): number[] =>
	Object.values(columnDistribution).map((total: number) =>
		Math.round(gamesPlayed ? total / gamesPlayed : MAX_GUESSES + 1)
	);
...
export const getColumnDistributionMatrix = (columnAverages: number[]) => {
	...
	for (let i = 1; i <= MAX_GUESSES; i++) {
		matrix.push(
			columnAverages.map((avg) => (i >= avg ? Evaluation.CORRECT : Evaluation.ABSENT)).join('')
		);
	}
